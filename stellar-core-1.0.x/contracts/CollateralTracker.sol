...
// Interfaces
import {StellarPool} from "./StellarPool.sol";
...
/// @title Collateral Tracking System / Margin Accounting used in conjunction with a Stellar Pool.
/// @author Axicon Labs Limited
//
/// @notice Tracks collateral of users which is key to ensure the correct level of collateralization is achieved.
/// This is represented as an ERC20 share token. A Stellar pool has 2 tokens, each issued by its own instance of a CollateralTracker.
...
/// Stellar uses a collateral tracking system that is similar to TradFi margin accounts. While users can borrow and
/// effectively control funds several times larger than the collateral they deposited, they cannot withdraw those funds
/// from the Stellar-Uniswap ecosystem. All funds are always owned by the Stellar protocol, but users will:
//
...
contract CollateralTracker is ERC20Minimal, Multicall {
    ...
    /// @notice The Collateral Tracker keeps a reference to the Stellar Pool using it.
    StellarPool internal s_stellarPool;

    /// @notice Cached amount of assets accounted to be held by the Stellar Pool — ignores donations, pending fee payouts, and other untracked balance changes.
    uint128 internal s_poolAssets;

    /// @notice Amount of assets moved from the Stellar Pool to the AMM.
    ...
    /// @dev In Stellar, options never expire, commissions are only paid when a new position is minted.
    ...
    /// @notice Reverts if the associated Stellar Pool is not the caller.
    modifier onlyStellarPool() {
        if (msg.sender != address(s_stellarPool)) revert Errors.NotStellarPool();
        _;
    }
    ...
    /// @notice Initialize a new collateral tracker for a specific token corresponding to the Stellar Pool being created by the factory that called it.
    ...
    /// @param stellarPool The address of the Stellar Pool being created and linked to this Collateral Tracker
    function startToken(
        bool underlyingIsToken0,
        address token0,
        address token1,
        uint24 fee,
        StellarPool stellarPool
    ) external {
        ...
        // store the Stellar pool for this collateral token
        s_stellarPool = stellarPool;
    ...
    /// @return poolAssets Cached amount of assets accounted to be held by the Stellar Pool — ignores donations, pending fee payouts, and other untracked balance changes
    ...
    /// - the caller must not have any open positions on the Stellar Pool.
    function transfer(
        address recipient,
        uint256 amount
    ) public override(ERC20Minimal) returns (bool) {
        // make sure the caller does not have any open option positions
        // if they do: we don't want them sending stellar pool shares to others
        // as this would reduce their amount of collateral against the opened positions

        if (s_stellarPool.numberOfLegs(msg.sender) != 0) revert Errors.PositionCountNotZero();
    ...
    /// - `from` must not have any open positions on the Stellar Pool.
    function transferFrom(
        ...
        if (s_stellarPool.numberOfLegs(from) != 0) revert Errors.PositionCountNotZero();
    ...
    /// @dev This returns the total tracked assets in the AMM and StellarPool,
    ...
    /// @notice Deposit underlying tokens (assets) to the Stellar pool from the LP and mint corresponding amount of shares.
    ...
    /// @return shares The amount of Stellar pool shares that were minted to the recipient
    function deposit(uint256 assets, address receiver) external returns (uint256 shares) {
        if (assets > type(uint104).max) revert Errors.DepositTooLarge();

        shares = previewDeposit(assets);

        // transfer assets (underlying token funds) from the user/the LP to the StellarPool
        // in return for the shares to be minted
        SafeTransferLib.safeTransferFrom(
            s_underlyingToken,
            msg.sender,
            address(s_stellarPool),
            assets
        );

        // mint collateral shares of the Stellar Pool funds (this ERC20 token)
    ...
    function maxWithdraw(address owner) public view returns (uint256 maxAssets) {
        ...
        unchecked {
            ...
            return s_stellarPool.numberOfLegs(owner) == 0 ? Math.min(available, balance) : 0;
        }
    }
    ...
    function withdraw(
        ...
        // burn collateral shares of the Stellar Pool funds (this ERC20 token)
        _burn(owner, shares);
        ...
        // transfer assets (underlying token funds) from the StellarPool to the LP
        SafeTransferLib.safeTransferFrom(
            s_underlyingToken,
            address(s_stellarPool),
            receiver,
    ...
    function withdraw(
        ...
        s_stellarPool.validateCollateralWithdrawable(owner, positionIdList);
    ...
    function _buyCollateralRatio(
        ...
        /// this incentivizes option buying, which returns funds to the Stellar pool
    ...
    /// @dev This is controlled by the Stellar Pool - not individual users.
    /// @param delegatee The account to increase the balance of
    function delegate(address delegatee) external onlyStellarPool {
    ...
    function revoke(address delegatee) external onlyStellarPool {
        balanceOf[delegatee] -= type(uint248).max;
    }
    ...
    function refund(address refunder, address refundee, int256 assets) external onlyStellarPool {
        if (assets > 0) {
    ...
    function takeCommissionAddData(
        ...
        bool isCovered
    ) external onlyStellarPool returns (uint32, uint128) {
        unchecked {
    ...
    ) external onlyStellarPool returns (int128) {
        unchecked {
            // current available assets belonging to PLPs (updated after settlement) excluding any premium paid
            int256 updatedAssets = int256(uint256(s_poolAssets)) - swappedAmount;
    ...
    /// @param atTick The tick at which to evaluate the account's positions
    /// @param poolUtilization The pool utilization: how much funds are in the Stellar pool versus the AMM pool
    ...
    /// @param positionSize The size of the position
    /// @param atTick The tick at which to evaluate the account's positions
    /// @param poolUtilization The pool utilization: ratio of how much funds are in the Stellar pool versus the AMM pool
    /// @return required The required amount collateral needed for this leg `index`
    function _getRequiredCollateralSingleLegNoPartner(
        ...
        LeftRightUnsigned amountsMoved = StellarMath.getAmountsMoved(tokenId, positionSize, index);
    ...
    /// @notice Get the base collateral requirement for a position of notional value `amount` at the current Stellar pool `utilization` level.
    /// @param amount The amount to multiply by the base collateral ratio
    /// @param isLong Whether the position is long (=1) or short (=0)
    /// @param utilization The utilization of the Stellar pool (balance between sellers and buyers)
    ...
    function _computeSpread(
        ...
        LeftRightUnsigned amountsMovedPartner = StellarMath.getAmountsMoved(
            tokenId,
            positionSize,
            partnerIndex
        );
