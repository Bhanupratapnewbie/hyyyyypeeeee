...
library InteractionHelper {
    /// @notice Function that performs approvals on behalf of the StellarPool for CollateralTracker and SemiFungiblePositionManager.
    ...
    function computeName(
        ...
        string memory symbol0 = StellarMath.safeERC20Symbol(token0);
        string memory symbol1 = StellarMath.safeERC20Symbol(token1);
        ...
        unchecked {
            return
                string.concat(
                    ...
                    symbol1,
                    " ",
                    StellarMath.uniswapFeeToString(fee)
                );
        }
    }
    ...
    function computeSymbol(
        ...
        return string.concat(prefix, StellarMath.safeERC20Symbol(token));
    }
