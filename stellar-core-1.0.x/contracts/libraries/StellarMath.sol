...
/// @title Compute general math quantities relevant to Stellar and AMM pool management.
/// @notice Contains Stellar-specific helpers and math functions.
...
library StellarMath {
    ...
    /// @notice Given an address to a Uniswap V3 pool, return its 64-bit ID as used in the `TokenId` of Stellar.
    // Example:
    ...
    function getLiquidityChunk(
        ...
        //
        //  However, in Stellar, each position has a asset parameter. The asset is the "basis" of the position.
        ...
        if (sqrtPriceX96 < Constants.FP96) {
            return (
                tokenData0.rightSlot() +
                    StellarMath.convert1to0(tokenData1.rightSlot(), sqrtPriceX96),
                tokenData0.leftSlot() +
                    StellarMath.convert1to0RoundingUp(tokenData1.leftSlot(), sqrtPriceX96)
            );
        }

        return (
            StellarMath.convert0to1(tokenData0.rightSlot(), sqrtPriceX96) + tokenData1.rightSlot(),
            StellarMath.convert0to1RoundingUp(tokenData0.leftSlot(), sqrtPriceX96) +
                tokenData1.leftSlot()
        );
    }
    ...
    /// @notice Compute the amount of funds that are moved to or removed from the Stellar Pool when `tokenId` is created.
    /// @param tokenId The option position identifier
        ...
        unchecked {
            ...
            if (!(paid0 > balance0 && paid1 > balance1)) {
                ...
                if ((paid0 > balance0)) {
                    ...
                    bonus1 += Math.min(
                        balance1 - paid1,
                        StellarMath.convert0to1(paid0 - balance0, atSqrtPriceX96)
                    );
                    bonus0 -= Math.min(
                        StellarMath.convert1to0(balance1 - paid1, atSqrtPriceX96),
                        paid0 - balance0
                    );
                }
                if ((paid1 > balance1)) {
                    ...
                    bonus0 += Math.min(
                        balance0 - paid0,
                        StellarMath.convert1to0(paid1 - balance1, atSqrtPriceX96)
                    );
                    bonus1 -= Math.min(
                        StellarMath.convert0to1(balance0 - paid0, atSqrtPriceX96),
                        paid1 - balance1
                    );
                }
            }
            ...
            for (uint256 i = 0; i < positionIdList.length; i++) {
                ...
                for (uint256 leg = 0; leg < tokenId.countLegs(); ++leg) {
                    if (
                        ...
                        emit StellarPool.PremiumSettled(
                            _liquidatee,
                            tokenId,
                            leg,
