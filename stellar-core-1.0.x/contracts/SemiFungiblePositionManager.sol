...
contract SemiFungiblePositionManager is ERC1155, Multicall, TransientReentrancyGuard {
    ...
    */
    ...
    function initializeAMMPool(address token0, address token1, uint24 fee) external {
        ...
        // pools can be initialized from the Stellar Factory or by calling initializeAMMPool directly, so reverting
        // could prevent a StellarPool from being deployed on a previously initialized but otherwise valid pool
        ...
        while (address(s_poolIdToPoolData[poolId].pool) != address(0)) {
            poolId = StellarMath.incrementPoolPattern(poolId);
        }
    ...
    function swapInAMM(
        ...
        unchecked {
            ...
            // NOTE: upstream users of this function such as the Stellar Pool should ensure users always compensate for the ITM amount delta
    ...
    function _burnLiquidity(
        ...
        // This will send the underlying tokens back to the Stellar Pool (msg.sender)
        ...
        // amount0 The amount of token0 that was sent back to the Stellar Pool
        // amount1 The amount of token1 that was sent back to the Stellar Pool
        // no need to safecast to int from uint here as the max position size is int128
