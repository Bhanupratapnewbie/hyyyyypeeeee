...
/// @title Stellar Factory which creates and registers Stellar Pools.
/// @author Axicon Labs Limited
/// @notice Facilitates deployment of Stellar pools.
contract StellarFactory is FactoryNFT, Multicall {
    /*//////////////////////////////////////////////////////////////
                                 EVENTS
    //////////////////////////////////////////////////////////////*/

    /// @notice Emitted when a Stellar Pool is created.
    /// @param poolAddress Address of the deployed Stellar pool
    ...
    event PoolDeployed(
        StellarPool indexed poolAddress,
    ...
    /// @notice The Semi Fungible Position Manager (SFPM) which tracks option positions across Stellar Pools.
    SemiFungiblePositionManager internal immutable SFPM;

    /// @notice Reference implementation of the `StellarPool` to clone.
    ...
    /// @notice The `observationCardinalityNext` to set on the Uniswap pool when a new StellarPool is deployed.
    ...
    /// @notice Mapping from address(UniswapV3Pool) to address(StellarPool) that stores the address of all deployed Stellar Pools.
    mapping(IUniswapV3Pool univ3pool => StellarPool stellarPool) internal s_getStellarPool;
    ...
    /// @param _poolReference The reference implementation of the `StellarPool` to clone
    ...
    /// @notice Create a new Stellar Pool linked to the given Uniswap pool identified uniquely by the incoming parameters.
    /// @dev There is a 1:1 mapping between a Stellar Pool and a Uniswap Pool.
    /// @dev A Uniswap pool is uniquely identified by its tokens and the fee.
    /// @dev Salt used in StellarPool CREATE2 is `[leading 20 msg.sender chars][leading 20 pool address chars][salt]`.
    ...
    /// @param salt User-defined component of salt used in CREATE2 for the StellarPool (must be a uint96 number)
    /// @return newPoolContract The address of the newly deployed Stellar pool
    function deployNewPool(
        address token0,
        address token1,
        uint24 fee,
        uint96 salt
    ) external returns (StellarPool newPoolContract) {
        ...
        if (address(s_getStellarPool[v3Pool]) != address(0))
        ...
        // This creates a new Stellar Pool (proxy to the StellarPool implementation)
        newPoolContract = StellarPool(POOL_REFERENCE.cloneDeterministic(salt32));
        ...
        s_getStellarPool[v3Pool] = newPoolContract;

        // The Stellar pool won't be safe to use until the observation cardinality is at least CARDINALITY_INCREASE
        // If this is not the case, we increase the next cardinality during deployment so the cardinality can catch up over time
        // When that happens, there will be a period of time where the StellarPool is deployed, but not (safely) usable
    ...
    /// @notice Find the salt which would give a Stellar Pool the highest rarity within the search parameters.
    /// @dev The rarity is defined in terms of how many leading zeros the Stellar pool address has.
    /// @dev Note that the final salt may overflow if too many loops are given relative to the amount in `salt`.
    /// @param deployerAddress Address of the account that deploys the new StellarPool
    ...
    function minePoolAddress(
        ...
        for (; uint256(salt) < maxSalt; ) {
            ...
            uint256 rarity = StellarMath.numberOfLeadingHexZeros(
    ...
    /// @notice Return the address of the Stellar Pool associated with `univ3pool`.
    /// @param univ3pool The Uniswap V3 pool address to query
    /// @return Address of the Stellar Pool associated with `univ3pool`
    function getStellarPool(IUniswapV3Pool univ3pool) external view returns (StellarPool) {
        return s_getStellarPool[univ3pool];
    }
}
