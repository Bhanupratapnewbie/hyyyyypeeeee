...
/// @title FactoryNFT: ERC721 contract for Stellar Factory NFTs.
/// @notice Constructs dynamic SVG art and metadata for Stellar Factory NFTs from a set of building blocks.
...
contract FactoryNFT is MetadataStore, ERC721 {
    ...
    constructor(
        ...
        ERC721("Stellar V1 Factory Deployer NFTs", "STELLAR-NFT")
    {}

    /// @notice Returns the metadata URI for a given `tokenId`.
    /// @dev The metadata is dynamically generated from the characteristics of the `StellarPool` encoded in `tokenId`.
    /// @dev The first 160 bits of `tokenId` are the address of a Stellar Pool.
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        address stellarPool = address(uint160(tokenId));

        return
            constructMetadata(
                stellarPool,
                StellarMath.safeERC20Symbol(StellarPool(stellarPool).univ3pool().token0()),
                StellarMath.safeERC20Symbol(StellarPool(stellarPool).univ3pool().token1()),
                StellarPool(stellarPool).univ3pool().fee()
            );
    }

    /// @notice Returns the metadata URI for a given set of characteristics.
    /// @param stellarPool The displayed address used to determine the rarity (leading zeros) and lastCharVal (last 4 bits)
    ...
    function constructMetadata(
        ...
        uint256 lastCharVal = uint160(stellarPool) & 0xF;
        uint256 rarity = StellarMath.numberOfLeadingHexZeros(stellarPool);

        string memory svgOut = generateSVGArt(lastCharVal, rarity);

        svgOut = generateSVGInfo(svgOut, stellarPool, rarity, symbol0, symbol1);
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                abi.encodePacked(
                                    LibString.toHexString(uint256(uint160(stellarPool)), 20),
                                    "-",
                                    string.concat(
                                ...
                                '", "description":"',
                                string.concat(
                                    "Stellar Pool for the ",
                                    symbol0,
                                    "-",
                                    symbol1,
                                    "-",
                                    StellarMath.uniswapFeeToString(uint24(fee)),
                                    " market"
                                ),
    ...
    /// @param stellarPool The address of the Stellar Pool
    /// @param rarity The rarity of the NFT
    /// @param symbol0 The symbol of `token0` in the Uniswap pool
    /// @param symbol1 The symbol of `token1` in the Uniswap pool
    ...
    function generateSVGInfo(
        ...
        svgIn = svgIn
            .replace("<!-- POOLADDRESS -->", LibString.toHexString(uint160(stellarPool), 20))
