...
contract StellarFactoryHarness is StellarFactory {
    constructor(
...
contract StellarFactoryTest is Test {
    // the instance of the Stellar Factory we are testing
    StellarFactoryHarness stellarFactory;
    ...
    // These assets are used to mint 'dead shares' to the Stellar Factory
    ...
    function _initalizeWorldState(IUniswapV3Pool _pool) internal {
        ...
        IERC20Partial(token0).approve(address(stellarFactory), INITIAL_MOCK_TOKENS);
        IERC20Partial(token1).approve(address(stellarFactory), INITIAL_MOCK_TOKENS);
    ...
    function setUp() public {
        ...
        // Deploy factory
        stellarFactory = new StellarFactoryHarness(
            sfpm,
            V3FACTORY,
            address(new StellarPool(sfpm)),
    ...
    function test_Success_deployNewPool(uint256 x, uint96 salt) public {
        ...
        // Compute clone determinsitic Stellar Factory address
        address poolReference = stellarFactory.getPoolReference();
        address preComputedPool = predictDeterministicAddress(
            ...
            address(stellarFactory)
        );

        {
            // Deploy pool
            // links the Uniswap V3 pool to the Stellar pool
            StellarPool deployedPool = stellarFactory.deployNewPool(token0, token1, fee, salt);
            ...
            // check if pool is linked to the correct stellar pool in factory
            assertEq(address(stellarFactory.getStellarPool(pool)), address(deployedPool));
            // see if correct pool was linked in the stellarPool
            IUniswapV3Pool linkedPool = StellarPool(preComputedPool).univ3pool();
            address linkedPoolAddress = address(StellarPool(preComputedPool).univ3pool());
    ...
    // Revert if trying to deploy a Stellar Pool ontop of an invalid Uniswap Pool
    function test_Fail_deployinvalidPool() public {
        ...
        stellarFactory.deployNewPool(token0, token1, fee, salt);
    }

    // Revert if deploying a Stellar Pool that has already been initalized
    function test_Fail_deployExistingPool() public {
        ...
        unchecked {
            stellarFactory.deployNewPool(token0, token1, fee, salt + 1);
        }
    }
    ...
    function test_Success_tokenURI_decodes() public {
        ...
        uint96 salt = uint96(block.timestamp);
        StellarPool deployedPool = stellarFactory.deployNewPool(token0, token1, fee, salt);
        uint256 stellarPoolAddress = uint256(uint160(address(deployedPool)));
        bytes memory uri = bytes(stellarFactory.tokenURI(stellarPoolAddress));
    ...
    // Successfully reach or surpass target rarity and deploy a Stellar pool with the mined 'bestSalt'
    function test_Success_mineTargetRarity(
        ...
        // mine pool address
        (uint96 bestSalt, uint256 highestRarity) = stellarFactory.minePoolAddress(
            randomAddress,
            address(pool),
            nonce,
            50_000,
        ...
        assertEq(
            highestRarity,
            StellarMath.numberOfLeadingHexZeros(
                predictDeterministicAddress(
                    stellarFactory.getPoolReference(),
                    bytes32(
                        abi.encodePacked(
