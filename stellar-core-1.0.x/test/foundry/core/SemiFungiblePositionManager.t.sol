...
contract SemiFungiblePositionManagerTest is PositionUtils {
    ...
    function test_Success_initializeAMMPool_Single(uint256 x) public {
        ...
        assertEq(
            address(
                sfpm.getUniswapV3PoolFromId(
                    StellarMath.getPoolId(address(pool), pool.tickSpacing())
                )
            ),
            address(pool)
        );
        ...
        assertEq(
            ...
            StellarMath.getPoolId(address(pool), pool.tickSpacing()) + 2 ** 255
        );
    }
    ...
    function test_Success_initializeAMMPool_HandleCollisions() public {
        ...
        for (uint160 i = 0; i < 100; i++) {
            ...
            if (i != 0) {
                poolIdNew = StellarMath.incrementPoolPattern(poolIdNew);
            }
