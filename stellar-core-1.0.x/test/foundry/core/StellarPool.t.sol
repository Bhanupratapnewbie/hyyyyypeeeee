...
import {StellarFactory} from "@contracts/StellarFactory.sol";
...
contract StellarPoolHarness is StellarPool {
    ...
    function getUniV3TWAP_() external view returns (int24 twapTick) {
        twapTick = StellarMath.twapFilter(s_univ3pool, TWAP_WINDOW);
    }
...
contract StellarPoolTest is PositionUtils {
    /*//////////////////////////////////////////////////////////////
                           MAINNET CONTRACTS
    //////////////////////////////////////////////////////////////*/
    ...
    StellarFactory factory;
    StellarPoolHarness pp;
    ...
    function _deployStellarPool() internal {
    ...
    function setUp() public {
        ...
        ph = new StellarHelper(sfpm);

        // deploy reference pool and collateral token
        poolReference = address(new StellarPoolHarness(sfpm));
    ...
    function populatePositionData(int24 width, int24 strike, uint256 positionSizeSeed) internal {
        (int24 rangeDown, int24 rangeUp) = StellarMath.getRangesFromStrike(width, tickSpacing);
    ...
    function populatePositionData(
        ...
        (int24 rangeDown0, int24 rangeUp0) = StellarMath.getRangesFromStrike(
            width[0],
            tickSpacing
        );
        ...
        (int24 rangeDown1, int24 rangeUp1) = StellarMath.getRangesFromStrike(
            width[1],
            tickSpacing
        );
    ...
    function populatePositionData(
        ...
        (int24 rangeDown2, int24 rangeUp2) = StellarMath.getRangesFromStrike(
            width[2],
            tickSpacing
        );
    ...
    function populatePositionData(
        ...
        (int24 rangeDown3, int24 rangeUp3) = StellarMath.getRangesFromStrike(
            width[3],
            tickSpacing
        );
    ...
    function populatePositionData(
        ...
        (int24 rangeDown0, int24 rangeUp0) = StellarMath.getRangesFromStrike(width, tickSpacing);
    ...
    function test_Success_mintOptions_OTMShortCall_SlippageSet(
        ...
        {
            (, LeftRightSigned shortAmounts) = StellarMath.computeExercisedAmounts(
                tokenId,
                uint128(positionSize)
            );
    ...
    function test_Success_mintOptions_ITMShortPutShortCall(
        ...
        int256 netSurplus0 = $amount0Moveds[0] -
            StellarMath.convert1to0($amount1Moveds[1], currentSqrtPriceX96);
        ...
        if (pp.isSafeMode() == false) {
            ...
            (fastOracleTick, ) = StellarMath.computeMedianObservedPrice(
                pool,
                observationIndex,
            ...
            (slowOracleTick, ) = StellarMath.computeInternalMedian(
                observationIndex,
                observationCardinality,
                60,
                pp.miniMedian(),
    ...
    function test_Success_mintOptions_ITMShortPutLongCall(
        ...
        (, LeftRightSigned shortAmountsSold) = StellarMath.computeExercisedAmounts(
            tokenId,
            positionSizes[0]
        );
        ...
        int256 netSurplus0 = $amount0Moveds[1] -
            StellarMath.convert1to0($amount1Moveds[2], currentSqrtPriceX96);
    ...
    function test_Fail_burnOptions_notEnoughCollateral(
        ...
        lastCollateralBalance1[Alice] = ct1.balanceOf(Alice);
        {
            ...
            (int24 _fastOracleTick, ) = StellarMath.computeMedianObservedPrice(
                pool,
                observationIndex,
                observationCardinality,
                3,
        ...
        if (fastOracleTick > 0)
            totalCollateralRequired0 = StellarMath.convert1to0(
    ...
    function test_Success_forceExerciseDelta(
        ...
        for (uint256 i = 0; i < numLegs; ++i) {
            ...
            int256 legRanges;
            {
                int24 rangeDown;
                int24 rangeUp;
                (rangeDown, rangeUp) = StellarMath.getRangesFromStrike(
                    widths[i],
                    int24(tickSpacing)
                );
    ...
    function test_Success_getExerciseDeltas(
        ...
        unchecked {
            ...
            LeftRightSigned refundAmounts = StellarMath.getExerciseDeltas(
                Charlie,
    ...
    function test_success_liquidate(
        ...
        $TSSubliquidateeBalancePre0 = ct0.totalSupply() - ct0.balanceOf(Alice);
        $TSSubliquidateeBalancePre1 = ct1.totalSupply() - ct1.balanceOf(Alice);
            ...
            for (uint256 i = 0; i < $posIdLists[1].length; ++i) {
                for (uint256 j = 0; j < $posIdLists[1][i].countLegs(); ++j) {
                    longPremium0 += premiasByLeg[i][j].rightSlot() < 0
                        ...
                        : int128(0);
                    longPremium0 += StellarMath.convert1to0(
...

        {
            ...
            (bonusAmounts, ) = StellarMath.getLiquidationBonus(
                $tokenData0,
                $tokenData1,
        ...
        for (uint256 i = 0; i < $posIdLists[1].length; ++i) {
            for (uint256 j = 0; j < $posIdLists[1][i].countLegs(); ++j) {
                ...
                settledTokens0[0] += StellarMath.convert1to0(
                    $settledTokens[chunk].leftSlot(),
                    TickMath.getSqrtRatioAtTick(TWAPtick)
                );
                settledTokens0[1] += StellarMath.convert1to0(
    ...
    function test_Fail_liquidate_NotMarginCalled(
        ...
        if (pp.getUniV3TWAP_() > 0)
            twapCollateralRequired0 = StellarMath.convert1to0(
        ...
        if (currentTick > 0)
            currentCollateralRequired0 = StellarMath.convert1to0(
        ...
        uint256 totalCollateralB0 = bound(
            ...
            uint256(
                Math.min(
                    int256(uint256(type(uint104).max)),
                    int256(StellarMath.convert1to0(type(uint104).max, currentSqrtPriceX96))
                )
            )
        );
