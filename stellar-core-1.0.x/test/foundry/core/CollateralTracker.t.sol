...
// Stellar Core
import {StellarPool} from "@contracts/StellarPool.sol";
import {SemiFungiblePositionManager} from "@contracts/SemiFungiblePositionManager.sol";
import {CollateralTracker} from "@contracts/CollateralTracker.sol";
import {StellarHelper} from "@test_periphery/StellarHelper.sol";

// Stellar Libraries
import {StellarMath} from "@libraries/StellarMath.sol";
import {Math} from "@libraries/StellarMath.sol";
...
contract CollateralTrackerHarness is CollateralTracker, PositionUtils, MiniPositionManager {
    ...
    // view deployer (stellar pool)
    function stellarPool() external view returns (StellarPool) {
        return s_stellarPool;
    }
...
// Inherits all of StellarPool's functionality, however uses a modified version of startPool
// which enables us to use our modified CollateralTracker harness that exposes internal data
contract StellarPoolHarness is StellarPool {
    constructor(SemiFungiblePositionManager _SFPM) StellarPool(_SFPM) {}
    ...
    function getTWAP() external view returns (int24 twapTick) {
        return StellarPool.getUniV3TWAP();
    }
...
contract CollateralTrackerTest is Test, PositionUtils {
    ...
    // Current instance of Stellar Pool, CollateralTokens, and SFPM
    StellarPoolHarness stellarPool;
    address stellarPoolAddress;
    StellarHelper stellarHelper;
    ...
    function _initWorld(uint256 seed) internal {
        ...
        _deployCustomStellarPool(token0, token1, pool);
    }

    function _cacheWorldState(IUniswapV3Pool _pool) internal {
        pool = _pool;
        poolId = StellarMath.getPoolId(address(_pool), _pool.tickSpacing());
        token0 = _pool.token0();
        token1 = _pool.token1();
    ...
    function _deployCustomStellarPool(
        address _token0,
        address _token1,
        IUniswapV3Pool uniswapPool
    ) internal {
        ...
        stellarHelper = new StellarHelper(SemiFungiblePositionManager(sfpm));

        // deploy modified Stellar pool
        stellarPool = new StellarPoolHarness(sfpm);

        // initalize Stellar Pool
        stellarPool.modifiedStartPool(_token0, _token1, uniswapPool);

        collateralToken0 = CollateralTrackerHarness(address(stellarPool.collateralToken0()));
        collateralToken1 = CollateralTrackerHarness(address(stellarPool.collateralToken1()));

        // store stellar pool address
        stellarPoolAddress = address(stellarPool);
    }
    ...
    function _mockMaxDeposit(address recipient) internal {
        ...
        deal(
            token0,
            address(stellarPool),
            IERC20Partial(token0).balanceOf(stellarPoolAddress) + initialMockTokens
        );
        deal(
            token1,
            address(stellarPool),
            IERC20Partial(token1).balanceOf(stellarPoolAddress) + initialMockTokens
        );
    }

    //@note move this and stellarPool helper into position utils
    /*//////////////////////////////////////////////////////////////
                                HELPERS
    //////////////////////////////////////////////////////////////*/
    ...
    function test_Success_StartToken_virtualShares() public {
        ...
        ct.startToken(false, token0, token1, fee, stellarPool);
    ...
    function test_Fail_startToken_alreadyInitializedToken(uint256 x) public {
        ...
        // initialize the token
        collateralToken0.startToken(false, token0, token1, fee, StellarPool(address(0)));
    ...
    function test_Success_deposit(uint256 x, uint104 assets) public {
        ...
        // check if the stellar pool got transferred the correct underlying assets
        assertEq(assets, IERC20Partial(underlyingToken0).balanceOf(address(stellarPool)));
        assertEq(assets, IERC20Partial(underlyingToken1).balanceOf(address(stellarPool)));
    }
    ...
    function test_Success_mint(uint256 x, uint104 shares) public {
        ...
        // check if the stellar pool got transferred the correct underlying assets
        assertEq(returnedAssets0, IERC20Partial(underlyingToken0).balanceOf(address(stellarPool)));
        assertEq(returnedAssets1, IERC20Partial(underlyingToken1).balanceOf(address(stellarPool)));
    }
    ...
    function test_Fail_mint_DepositTooLarge(uint256 x, uint256 shares) public {
        ...
        IERC20Partial(token0).transfer(address(stellarPool), 2 ** 64);
        IERC20Partial(token1).transfer(address(stellarPool), 2 ** 64);
    ...
    function test_Fail_transfer_positionCountNotZero(
        ...
        // deposit a significant amount of assets into the Stellar pool
        _mockMaxDeposit(Bob);
    ...
    function test_Success_redeem(uint256 x, uint104 shares) public {
        ...
        uint256 debitedBalance1;
        {
            ...
            // Bob's asset balance after depositing to the Stellar pool
        ...
        // check correct amount of assets were moved from the the Stellar Pool to LP
    ...
    function test_success_delegate_virtual(uint256 x) public {
        ...
        stellarPool.delegate(Alice, collateralToken0);
        stellarPool.delegate(Alice, collateralToken1);
    ...
    function test_success_revoke_virtual(uint256 x) public {
        ...
        stellarPool.revoke(Alice, collateralToken0);
        stellarPool.revoke(Alice, collateralToken1);
    ...
    function test_success_refund_positive(uint256 x, uint104 shares) public {
        ...
        // invoke delegate transactions from the Stellar pool
        stellarPool.refund(
            Alice,
            Bob,
    ...
    function test_Fail_All_OnlyStellarPool(uint256 x, address caller) public {
        _initWorld(x);
        vm.assume(caller != address(stellarPool));

        vm.prank(caller);

        vm.expectRevert(Errors.NotStellarPool.selector);
    ...
    function test_Success_collateralCheck_shortStrangle(
...

        {
            ...
            // initalize a custom Stellar pool
            _deployCustomStellarPool(token0, token1, pool);
...

        {
            ...
            ($shortPremia, $longPremia, posBalanceArray) = stellarPool
            ...
            (, uint64 poolUtilization0, uint64 poolUtilization1) = stellarHelper
                .optionPositionInfo(stellarPool, Alice, tokenId1);
...

        {
            ...
            (uint256 calcBalanceCross, uint256 calcThresholdCross) = StellarMath.getCrossBalances(
                tokenData0,
                tokenData1,
                Math.getSqrtRatioAtTick(currentTick)
            );

            (balanceData0, thresholdData0) = stellarHelper.checkCollateral(
                stellarPool,
                Alice,
                currentTick,
                positionIdList1
    ...
    function test_Success_collateralCheck_ITMcallSpread_assetTT1(
...

        {
            ...
            (rangeDown0, rangeUp0) = StellarMath.getRangesFromStrike(width, tickSpacing);

            (rangeDown1, rangeUp1) = StellarMath.getRangesFromStrike(width1, tickSpacing);
    ...
    function test_Success_collateralCheck_sellCallMinUtilization(
        ...
        // check requirement at fuzzed tick
        {
            ...
            (, uint64 poolUtilization0, uint64 poolUtilization1) = stellarHelper
                .optionPositionInfo(stellarPool, Bob, tokenId);
        ...
        //check collateral output against stellar pool at current tick
        {
            (, currentTick, , , , , ) = pool.slot0();
    ...
    function test_Success_exerciseCostRanges_OTMCall(
        ...
        // check requirement at fuzzed tick
        {
            ...
            (, int24 rangeUp) = StellarMath.getRangesFromStrike(width, tickSpacing);

            // strike - rangeDown
            vm.assume(atTick < legLowerTick);

            (LeftRightSigned longAmounts, ) = StellarMath.computeExercisedAmounts(
                tokenId1,
                positionSize0 / 4
            );
    ...
    function setUtilization(
        ...
        // totalAssets() = StellarPoolBal - lockedFunds + inAMM
        //        totalAssets() = z + inAMM
        //
        //
        // DECIMALS = 10_000
        //
        // utilization = (inAMM * DECIMALS) / ((StellarPoolBal - lockedFunds) + inAMM)
        //      z = (StellarPoolBal - lockedFunds)
        //      utilization =  (inAMM * DECIMALS) / z + inAMM
        //      inAMM = (utilization * z) / (10_000 - utilization)
        //      inAMM / z = utilization / (10_000 - utilization)
        //
        // inAMM / (StellarPoolBal - lockedFunds) = (utilization / (10_000(DECIMALS) - utilization))
        //
        // i.e utilization of 9_000
        //    inAMM / (StellarPoolBal - lockedFunds) = 9_000 / 10_000 - 9_000
        //    inAMM / (StellarPoolBal - lockedFunds) = 9
        ...
        //
        //-----------------------------------------------------------
        int128 _poolBalance = int128(int256(IERC20Partial(token).balanceOf(address(stellarPool))));
        ...
        // inAMM = utilization * (StellarPoolBal - lockedFunds) / (10_000 - utilization)
        ...
        deal(token, address(stellarPool), uint128(_poolBalance));
    }
    ...
    function test_Success_maxRedeem(uint256 x, uint104 shares) public {
        ...
        if (stellarPool.numberOfLegs(Bob) != 0) {
    ...
    function test_Success_availableAssets(uint256 x, uint256 balance) public {
        ...
        // set total balance of underlying asset in the Stellar pool
    ...
    function _swapITM(int128 itm0, int128 itm1) internal {
        ...
        if ((itm0 != 0) && (itm1 != 0)) {
            ...
            int256 net0 = itm0 + StellarMath.convert1to0(itm1, sqrtPriceX96);

            int256 net1 = itm1 + StellarMath.convert0to1(itm0, sqrtPriceX96);
    ...
    function _tokensRequired(
        ...
        for (; i < maxLoop; ++i) {
            ...
            (int24 rangeDown, int24 rangeUp) = StellarMath.getRangesFromStrike(
                _width,
                tickSpacing
            );
            ...
            {
                LeftRightUnsigned _amountsMoved = StellarMath.getAmountsMoved(
                    _tokenId,
                    positionSize,
                    i
                );
        ...
        for (uint i; i < maxLoop; ++i) {
...

            {
                if (isLong == 1) {
                    // spread requirement
                    {
                        amountsMoved = StellarMath.getAmountsMoved(_tokenId, positionSize, i);

                        amountsMovedPartner = StellarMath.getAmountsMoved(
                            _tokenId,
                            positionSize,
                            partnerIndex
                        );
    ...
    function _strangleTokensRequired(
        ...
        for (uint i; i < maxLoop; ++i) {
            ...
            (, rangeUp0) = StellarMath.getRangesFromStrike(width, tickSpacing);
