...
contract StellarHelper {
    ...
    /// @notice Construct the StellarHelper contract
    ...
    /// @param pool The StellarPool instance to check collateral on
    ...
    function checkCollateral(
        ...
        return
            StellarMath.getCrossBalances(tokenData0, tokenData1, Math.getSqrtRatioAtTick(atTick));
    }
    ...
    /// @param pool The StellarPool instance corresponding to the pool specified in `TokenId`
    ...
    /// @return poolUtilization0 The utilization of token0 in the Stellar pool at mint
    /// @return poolUtilization1 The utilization of token1 in the Stellar pool at mint
    ...
    function computeMedianObservedPrice(
        ...
        (int24 medianTick, ) = StellarMath.computeMedianObservedPrice(
            univ3pool,
            observationIndex,
    ...
    function computeInternalMedian(
        ...
        (int24 _medianTick, uint256 _medianData) = StellarMath.computeInternalMedian(
            observationIndex,
            observationCardinality,
            period,
            medianData,
    ...
    function twapFilter(IUniswapV3Pool univ3pool, uint32 twapWindow) external view returns (int24) {
        return StellarMath.twapFilter(univ3pool, twapWindow);
    }
    ...
    function netEquity(
        ...
        if (tick > 0) {
            balanceCross = StellarMath.convert1to0(balanceCross, Math.getSqrtRatioAtTick(tick));
            requiredCross = StellarMath.convert1to0(requiredCross, Math.getSqrtRatioAtTick(tick));
        }
    ...
    function findLiquidationPriceDown(
        ...
        (, int24 currentTick, , , , , ) = StellarPool(pool).univ3pool().slot0();
