<p align="center">
  <img src="assets/Smart Contracts_1.png" width="1000" title="Stellar Banner"></img>
</p>
...
[![Twitter](https://img.shields.io/twitter/url/https/twitter.com/cloudposse.svg?style=social&label=Follow%20%40stellar)](https://twitter.com/stellar)

Stellar is a permissionless options trading protocol. It enables the trading of perpetual options on top of any [Uniswap V3](https://uniswap.org/) pool

The Stellar protocol is noncustodial, has no counterparty risk, offers instantaneous settlement, and is designed to remain fully collateralized at all times.

- [Stellar's Website](https://www.stellar.xyz)
- [Whitepaper](https://paper.stellar.xyz/)
- [Litepaper](https://intro.stellar.xyz)
- [Documentation](https://docs.stellar.xyz/)
- [Twitter](https://twitter.com/stellar)
- [Discord](https://discord.gg/7fE8SN9pRT)
- [Blog](https://www.stellar.xyz/blog)
- [YouTube](https://www.youtube.com/@stellar)

## Further Reading

Stellar has been presented at conferences and was conceived with the first Stellar's Genesis blog post in mid-summer 2021:

- [Stellar @ EthCC 2023](https://www.youtube.com/watch?v=9ubpnQRvxY8)
- [Stellar @ ETH Denver 2023](https://www.youtube.com/watch?v=Dt5AdCNavjs)
- [Stellar @ ETH Denver 2022](https://www.youtube.com/watch?v=mtd4JphPcuA)
- [Stellar @ DeFi Guild](https://www.youtube.com/watch?v=vlPIFYfG0FU)
- [Stellar's Genesis: Blog Series](https://lambert-guillaume.medium.com/)
...
All protocol users first onboard by depositing tokens into one or both CollateralTracker vaults and being issued shares (becoming PLPs in the process). Stellar’s CollateralTracker supports the full ERC4626 interface, making deposits and withdrawals a simple and standardized process. Passive PLPs stop here.
...
```ml
contracts/
├── CollateralTracker - "ERC4626 vault where token liquidity from Stellar Liquidity Providers (PLPs) and collateral for option positions are deposited and collateral requirements are computed"
├── StellarFactory - "Handles deployment of new Stellar instances on top of Uniswap pools, initial liquidity deployments, and NFT rewards for deployers"
├── StellarPool - "Coordinates all options trading activity - minting, burning, force exercises, liquidations"
├── SemiFungiblePositionManager - "The 'engine' of Stellar - manages all Uniswap V3 positions in the protocol as well as being a more advanced, gas-efficient alternative to NFPM for Uniswap LPs"
├── base
│   ├── FactoryNFT - "Constructs dynamic SVG art and metadata for Stellar Factory NFTs from a set of building blocks"
...
│   └── interfaces
│       └── IERC20Partial - "An incomplete ERC20 interface containing functions used in Stellar with some return values omitted to support noncompliant tokens such as USDT"
├── types
...
│   └── TokenId - "Implementation for the custom data type used in the SFPM and Stellar to encode position data in 256-bit ERC1155 tokenIds - holds a pool identifier and up to four full position legs"
└── libraries
    ├── CallbackLib - "Library for verifying and decoding Uniswap callbacks"
    ├── Constants - "Library of Constants used in Stellar"
    ├── Errors - "Contains all custom errors used in Stellar's core contracts"
    ...
    ├── StellarMath - "Library containing advanced Stellar/Uniswap-specific functionality such as our TWAP, price conversions, and position sizing math"
    └── SafeTransferLib - "Safe ERC20 transfer library that gracefully handles missing return values"
```

## Installation

Stellar uses the Foundry framework for testing and deployment, and Prettier for linting.
...
Stellar can also be deployed at a preconfigured set of addresses on mainnet. The canonical initcodes are saved in `deployment-info.json` and can be updated by running the following commands:
...
Each `address` and `salt` pair specified by `build-config.json` can be deployed through the `CREATE3` factory at `0x000000000000b361194cfe6312EE3210d53C15AA`. All salts in the current configuration are owned by `vault.stellar.eth`.

The following command can be run to generate the Safe transaction batches required to deploy Stellar with the code in `deployment-info.json` from a Safe multisig that owns salts:
...
Stellar can be deployed on any chain with a Uniswap V3 instance. To go through with the deployment, several environment variables need to be set:

- `DEPLOYER_PRIVATE_KEY` The private key of the EOA deploying the contracts
- `UNISWAP_V3_FACTORY` The address of the Uniswap V3 Factory Stellar is being deployed on
- `WETH9` The canonical Wrapped Ether deployment on the chain

To deploy Stellar for testing purposes, run:
...
The primary license for Stellar V1 is the Business Source License 1.1 (`BUSL-1.1`), see [LICENSE](https://github.com/Bhanupratapnewbie/hyyyyypeeeee/blob/main/LICENSE). Minus the following exceptions:
