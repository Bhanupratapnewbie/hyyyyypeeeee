...
/// @title The Growi Pool: Create permissionless options on a CLAMM.
/// @author Axicon Labs Limited
/// @notice Manages positions, collateral, liquidations and forced exercises.
contract GrowiPool is ERC1155Holder, Multicall {
    /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/
    ...
    /// @notice The "engine" of Growi - manages AMM liquidity and executes all mints/burns/exercises.
    ...
    /// @notice The Uniswap V3 pool that this instance of Growi is deployed on.
    ...
    /// @notice Initializes a Growi Pool on top of an existing Uniswap V3 + collateral vault pair.
    ...
    function pokeMedian() external {
        ...
        (, uint256 medianData) = GrowiMath.computeInternalMedian(
            observationIndex,
    ...
    function _mintOptions(
        ...
        // make sure the tokenId is for this Growi pool
        ...
        uint96 tickData;
        {
            (
                ...
                uint256 medianData
            ) = GrowiMath.getOracleTicks(s_univ3pool, s_miniMedian);
    ...
    /// @param tickLimitLow The lower bound of an acceptable open interval for the ending price
    /// @param tickLimitHigh The upper bound of an acceptable open interval for the ending price
    /// @return poolUtilizations Packing of the pool utilization (how much funds are in the Growi pool versus the AMM pool) at the time of minting,
    ...
    /// @return Packing of the pool utilization (how much funds are in the Growi pool versus the AMM pool at the time of minting),
    /// right 64bits for token0 and left 64bits for token1, defined as `(inAMM * 10_000) / totalAssets()`
    /// where totalAssets is the total tracked assets in the AMM and GrowiPool minus fees and donations to the Growi pool
    ...
    function liquidate(
        ...
        int24 currentTick;
        {
            ...
            int24 fastOracleTick;
            int24 lastObservedTick;
            (currentTick, fastOracleTick, , lastObservedTick, ) = GrowiMath.getOracleTicks(
                s_univ3pool,
                s_miniMedian
            );
        ...
        LeftRightSigned bonusAmounts;
        {
            ...
            (bonusAmounts, collateralRemaining) = GrowiMath.getLiquidationBonus(
                tokenData0,
                tokenData1,
            ...
            LeftRightSigned bonusDeltas = GrowiMath.haircutPremia(
                _liquidatee,
                _positionIdList,
                premiasByLeg,
                collateralRemaining,
    ...
    function _isAccountSolvent(
        ...
        (uint256 balanceCross, uint256 thresholdCross) = GrowiMath.getCrossBalances(
            tokenData0,
            tokenData1,
    ...
    function _validatePositionList(
        ...
        for (uint256 i = 0; i < pLength; ) {
            fingerprintIncomingList = GrowiMath.updatePositionsHash(
    ...
    function _updatePositionsHash(address account, TokenId tokenId, bool addFlag) internal {
        ...
        uint256 newHash = GrowiMath.updatePositionsHash(
    ...
    /// @notice Get the address of the AMM pool connected to this Growi pool.
    /// @return AMM pool corresponding to this Growi pool
    ...
    {
        (currentTick, fastOracleTick, slowOracleTick, latestObservation, ) = GrowiMath
