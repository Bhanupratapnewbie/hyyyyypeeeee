...
/// @title Growi Factory which creates and registers Growi Pools.
/// @author Axicon Labs Limited
/// @notice Facilitates deployment of Growi pools.
contract GrowiFactory is FactoryNFT, Multicall {
    /*//////////////////////////////////////////////////////////////
                                 EVENTS
    //////////////////////////////////////////////////////////////*/

    /// @notice Emitted when a Growi Pool is created.
    /// @param poolAddress Address of the deployed Growi pool
    ...
    /// @notice The Semi Fungible Position Manager (SFPM) which tracks option positions across Growi Pools.
    SemiFungiblePositionManager internal immutable SFPM;

    /// @notice Reference implementation of the `GrowiPool` to clone.
    ...
    /// @notice The `observationCardinalityNext` to set on the Uniswap pool when a new GrowiPool is deployed.
    ...
    /// @notice Mapping from address(UniswapV3Pool) to address(GrowiPool) that stores the address of all deployed Growi Pools.
    mapping(IUniswapV3Pool univ3pool => GrowiPool growiPool) internal s_getGrowiPool;
    ...
    /// @param _poolReference The reference implementation of the `GrowiPool` to clone
    ...
    /// @notice Create a new Growi Pool linked to the given Uniswap pool identified uniquely by the incoming parameters.
    /// @dev There is a 1:1 mapping between a Growi Pool and a Uniswap Pool.
    /// @dev A Uniswap pool is uniquely identified by its tokens and the fee.
    /// @dev Salt used in GrowiPool CREATE2 is `[leading 20 msg.sender chars][leading 20 pool address chars][salt]`.
    ...
    /// @param salt User-defined component of salt used in CREATE2 for the GrowiPool (must be a uint96 number)
    /// @return newPoolContract The address of the newly deployed Growi pool
    function deployNewPool(
        address token0,
        address token1,
        uint24 fee,
        uint96 salt
    ) external returns (GrowiPool newPoolContract) {
        ...
        if (address(s_getGrowiPool[v3Pool]) != address(0))
        ...
        // This creates a new Growi Pool (proxy to the GrowiPool implementation)
        newPoolContract = GrowiPool(POOL_REFERENCE.cloneDeterministic(salt32));
        ...
        s_getGrowiPool[v3Pool] = newPoolContract;

        // The Growi pool won't be safe to use until the observation cardinality is at least CARDINALITY_INCREASE
        // If this is not the case, we increase the next cardinality during deployment so the cardinality can catch up over time
        // When that happens, there will be a period of time where the GrowiPool is deployed, but not (safely) usable
    ...
    /// @notice Find the salt which would give a Growi Pool the highest rarity within the search parameters.
    /// @dev The rarity is defined in terms of how many leading zeros the Growi pool address has.
    /// @dev Note that the final salt may overflow if too many loops are given relative to the amount in `salt`.
    /// @param deployerAddress Address of the account that deploys the new GrowiPool
    ...
    function minePoolAddress(
        ...
        for (; uint256(salt) < maxSalt; ) {
            ...
            uint256 rarity = GrowiMath.numberOfLeadingHexZeros(
    ...
    /// @notice Return the address of the Growi Pool associated with `univ3pool`.
    /// @param univ3pool The Uniswap V3 pool address to query
    /// @return Address of the Growi Pool associated with `univ3pool`
    function getGrowiPool(IUniswapV3Pool univ3pool) external view returns (GrowiPool) {
        return s_getGrowiPool[univ3pool];
    }
}
