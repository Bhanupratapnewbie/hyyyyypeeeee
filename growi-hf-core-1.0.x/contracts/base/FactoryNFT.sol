...
/// @title FactoryNFT: ERC721 contract for Growi Factory NFTs.
/// @notice Constructs dynamic SVG art and metadata for Growi Factory NFTs from a set of building blocks.
...
contract FactoryNFT is MetadataStore, ERC721 {
    ...
    constructor(
        ...
        ERC721("Growi V1 Factory Deployer NFTs", "GROWI-NFT")
    {}

    /// @notice Returns the metadata URI for a given `tokenId`.
    /// @dev The metadata is dynamically generated from the characteristics of the `GrowiPool` encoded in `tokenId`.
    /// @dev The first 160 bits of `tokenId` are the address of a Growi Pool.
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        address growiPool = address(uint160(tokenId));

        return
            constructMetadata(
                growiPool,
                GrowiMath.safeERC20Symbol(GrowiPool(growiPool).univ3pool().token0()),
                GrowiMath.safeERC20Symbol(GrowiPool(growiPool).univ3pool().token1()),
                GrowiPool(growiPool).univ3pool().fee()
            );
    }

    /// @notice Returns the metadata URI for a given set of characteristics.
    /// @param growiPool The displayed address used to determine the rarity (leading zeros) and lastCharVal (last 4 bits)
    ...
    function constructMetadata(
        ...
        uint256 lastCharVal = uint160(growiPool) & 0xF;
        uint256 rarity = GrowiMath.numberOfLeadingHexZeros(growiPool);

        string memory svgOut = generateSVGArt(lastCharVal, rarity);

        svgOut = generateSVGInfo(svgOut, growiPool, rarity, symbol0, symbol1);
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                abi.encodePacked(
                                    LibString.toHexString(uint256(uint160(growiPool)), 20),
                                    "-",
                                    string.concat(
                                ...
                                string.concat(
                                    ...
                                    symbol1,
                                    "-",
                                    GrowiMath.uniswapFeeToString(uint24(fee)),
                                    " market"
                                ),
    ...
    /// @param growiPool The address of the Growi Pool
    /// @param rarity The rarity of the NFT
    /// @param symbol0 The symbol of `token0` in the Uniswap pool
    /// @param symbol1 The symbol of `token1` in the Uniswap pool
    ...
    function generateSVGInfo(
        ...
        svgIn = svgIn
            .replace("<!-- POOLADDRESS -->", LibString.toHexString(uint160(growiPool), 20))
