...
// Interfaces
import {GrowiPool} from "./GrowiPool.sol";
...
/// @title Collateral Tracking System / Margin Accounting used in conjunction with a Growi Pool.
/// @author Axicon Labs Limited
//
/// @notice Tracks collateral of users which is key to ensure the correct level of collateralization is achieved.
/// This is represented as an ERC20 share token. A Growi pool has 2 tokens, each issued by its own instance of a CollateralTracker.
...
/// Growi uses a collateral tracking system that is similar to TradFi margin accounts. While users can borrow and
/// effectively control funds several times larger than the collateral they deposited, they cannot withdraw those funds
/// from the Growi-Uniswap ecosystem. All funds are always owned by the Growi protocol, but users will:
//
...
contract CollateralTracker is ERC20Minimal, Multicall {
    ...
    /// @notice The Collateral Tracker keeps a reference to the Growi Pool using it.
    GrowiPool internal s_growiPool;

    /// @notice Cached amount of assets accounted to be held by the Growi Pool — ignores donations, pending fee payouts, and other untracked balance changes.
    uint128 internal s_poolAssets;

    /// @notice Amount of assets moved from the Growi Pool to the AMM.
    ...
    /// @dev In Growi, options never expire, commissions are only paid when a new position is minted.
    ...
    /// @notice Reverts if the associated Growi Pool is not the caller.
    modifier onlyGrowiPool() {
        if (msg.sender != address(s_growiPool)) revert Errors.NotGrowiPool();
        _;
    }
    ...
    /// @notice Initialize a new collateral tracker for a specific token corresponding to the Growi Pool being created by the factory that called it.
    ...
    /// @param growiPool The address of the Growi Pool being created and linked to this Collateral Tracker
    function startToken(
        ...
        // store the Growi pool for this collateral token
        s_growiPool = growiPool;
    ...
    /// @return poolAssets Cached amount of assets accounted to be held by the Growi Pool — ignores donations, pending fee payouts, and other untracked balance changes
    ...
    /// - the caller must not have any open positions on the Growi Pool.
    function transfer(
        address recipient,
        uint256 amount
    ) public override(ERC20Minimal) returns (bool) {
        // make sure the caller does not have any open option positions
        // if they do: we don't want them sending growi pool shares to others
        // as this would reduce their amount of collateral against the opened positions

        if (s_growiPool.numberOfLegs(msg.sender) != 0) revert Errors.PositionCountNotZero();
    ...
    /// - `from` must not have any open positions on the Growi Pool.
    function transferFrom(
        ...
        if (s_growiPool.numberOfLegs(from) != 0) revert Errors.PositionCountNotZero();
    ...
    /// @dev This returns the total tracked assets in the AMM and GrowiPool,
    ...
    /// @notice Deposit underlying tokens (assets) to the Growi pool from the LP and mint corresponding amount of shares.
    ...
    /// @return shares The amount of Growi pool shares that were minted to the recipient
    function deposit(uint256 assets, address receiver) external returns (uint256 shares) {
        if (assets > type(uint104).max) revert Errors.DepositTooLarge();

        shares = previewDeposit(assets);

        // transfer assets (underlying token funds) from the user/the LP to the GrowiPool
        ...
        // mint collateral shares of the Growi Pool funds (this ERC20 token)
    ...
    function maxWithdraw(address owner) public view returns (uint256 maxAssets) {
        ...
        unchecked {
            ...
            return s_growiPool.numberOfLegs(owner) == 0 ? Math.min(available, balance) : 0;
        }
    }
    ...
    function withdraw(
        ...
        // burn collateral shares of the Growi Pool funds (this ERC20 token)
        _burn(owner, shares);
        ...
        // transfer assets (underlying token funds) from the GrowiPool to the LP
        SafeTransferLib.safeTransferFrom(
            s_underlyingToken,
            address(s_growiPool),
            receiver,
    ...
    function withdraw(
        ...
        s_growiPool.validateCollateralWithdrawable(owner, positionIdList);
    ...
    function _buyCollateralRatio(
        ...
        /// this incentivizes option buying, which returns funds to the Growi pool
    ...
    /// @dev This is controlled by the Growi Pool - not individual users.
    /// @param delegatee The account to increase the balance of
    function delegate(address delegatee) external onlyGrowiPool {
    ...
    function revoke(address delegatee) external onlyGrowiPool {
        balanceOf[delegatee] -= type(uint248).max;
    }
    ...
    function refund(address refunder, address refundee, int256 assets) external onlyGrowiPool {
        if (assets > 0) {
    ...
    function takeCommissionAddData(
        ...
        bool isCovered
    ) external onlyGrowiPool returns (uint32, uint128) {
        unchecked {
    ...
    ) external onlyGrowiPool returns (int128) {
        unchecked {
            // current available assets belonging to PLPs (updated after settlement) excluding any premium paid
            int256 updatedAssets = int256(uint256(s_poolAssets)) - swappedAmount;
    ...
    /// @param atTick The tick at which to evaluate the account's positions
    /// @param poolUtilization The pool utilization: how much funds are in the Growi pool versus the AMM pool
    ...
    /// @param positionSize The size of the position
    /// @param atTick The tick at which to evaluate the account's positions
    /// @param poolUtilization The pool utilization: ratio of how much funds are in the Growi pool versus the AMM pool
    /// @return required The required amount collateral needed for this leg `index`
    function _getRequiredCollateralSingleLegNoPartner(
        ...
        LeftRightUnsigned amountsMoved = GrowiMath.getAmountsMoved(tokenId, positionSize, index);
    ...
    /// @notice Get the base collateral requirement for a position of notional value `amount` at the current Growi pool `utilization` level.
    /// @param amount The amount to multiply by the base collateral ratio
    /// @param isLong Whether the position is long (=1) or short (=0)
    /// @param utilization The utilization of the Growi pool (balance between sellers and buyers)
    ...
    function _computeSpread(
        ...
        LeftRightUnsigned amountsMovedPartner = GrowiMath.getAmountsMoved(
            tokenId,
            positionSize,
            partnerIndex
        );
