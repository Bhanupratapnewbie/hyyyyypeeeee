...
/// @notice Contains all custom error messages used in Growi.
library Errors {
    /// @notice GrowiPool: The account is not solvent enough to perform the desired action
    ...
    /// @notice GrowiPool: The effective liquidity (X32) is greater than min(`MAX_SPREAD`, `USER_PROVIDED_THRESHOLD`) during a long mint or short burn
    ...
    /// @notice GrowiPool: The provided list of option positions is incorrect or invalid
    error InputListFail();
    ...
    /// @notice GrowiPool: None of the legs in a position are force-exercisable (they are all either short or ATM long)
    error NoLegsExercisable();

    /// @notice GrowiPool: The leg is not long, so premium cannot be settled through `settleLongPremium`
    error NotALongLeg();

    /// @notice GrowiPool: There is not enough available liquidity in the chunk for one of the long legs to be created (or for one of the short legs to be closed)
    error NotEnoughLiquidity();

    /// @notice GrowiPool: Position is still solvent and cannot be liquidated
    error NotMarginCalled();

    /// @notice CollateralTracker: The caller for a permissioned function is not the Growi Pool
    error NotGrowiPool();

    /// @notice Uniswap pool has already been initialized in the SFPM or created in the factory
    error PoolAlreadyInitialized();

    /// @notice GrowiPool: A position with the given token ID has already been minted by the caller and is still open
    ...
    /// @notice GrowiPool: The position being minted would increase the total amount of legs open for the account above the maximum
    ...
    /// @notice The Uniswap Pool has not been created, so it cannot be used in the SFPM or have a GrowiPool created for it by the factory
