...
/// @title Compute general math quantities relevant to Growi and AMM pool management.
/// @notice Contains Growi-specific helpers and math functions.
...
library GrowiMath {
    ...
    /// @notice Given an address to a Uniswap V3 pool, return its 64-bit ID as used in the `TokenId` of Growi.
    // Example:
    ...
    function getLiquidityChunk(
        ...
        //
        //  However, in Growi, each position has a asset parameter. The asset is the "basis" of the position.
        ...
        if (sqrtPriceX96 < Constants.FP96) {
            return (
                tokenData0.rightSlot() +
                    GrowiMath.convert1to0(tokenData1.rightSlot(), sqrtPriceX96),
                tokenData0.leftSlot() +
                    GrowiMath.convert1to0RoundingUp(tokenData1.leftSlot(), sqrtPriceX96)
            );
        }

        return (
            GrowiMath.convert0to1(tokenData0.rightSlot(), sqrtPriceX96) + tokenData1.rightSlot(),
            GrowiMath.convert0to1RoundingUp(tokenData0.leftSlot(), sqrtPriceX96) +
                tokenData1.leftSlot()
        );
    }
    ...
    /// @notice Compute the amount of funds that are moved to or removed from the Growi Pool when `tokenId` is created.
    /// @param tokenId The option position identifier
        ...
        unchecked {
            ...
            if (!(paid0 > balance0 && paid1 > balance1)) {
                ...
                if ((paid0 > balance0)) {
                    ...
                    bonus1 += Math.min(
                        balance1 - paid1,
                        GrowiMath.convert0to1(paid0 - balance0, atSqrtPriceX96)
                    );
                    bonus0 -= Math.min(
                        GrowiMath.convert1to0(balance1 - paid1, atSqrtPriceX96),
                        paid0 - balance0
                    );
                }
                if ((paid1 > balance1)) {
                    ...
                    bonus0 += Math.min(
                        balance0 - paid0,
                        GrowiMath.convert1to0(paid1 - balance1, atSqrtPriceX96)
                    );
                    bonus1 -= Math.min(
                        GrowiMath.convert0to1(balance0 - paid0, atSqrtPriceX96),
                        paid1 - balance1
                    );
                }
            }
