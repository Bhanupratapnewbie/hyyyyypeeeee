...
library InteractionHelper {
    /// @notice Function that performs approvals on behalf of the GrowiPool for CollateralTracker and SemiFungiblePositionManager.
    ...
    function computeName(
        ...
        string memory symbol0 = GrowiMath.safeERC20Symbol(token0);
        string memory symbol1 = GrowiMath.safeERC20Symbol(token1);
        ...
        unchecked {
            return
                string.concat(
                    ...
                    symbol1,
                    " ",
                    GrowiMath.uniswapFeeToString(fee)
                );
        }
    }
    ...
    function computeSymbol(
        ...
        return string.concat(prefix, GrowiMath.safeERC20Symbol(token));
    }
