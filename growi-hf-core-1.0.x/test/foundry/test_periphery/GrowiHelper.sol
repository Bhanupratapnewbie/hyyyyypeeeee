...
contract GrowiHelper {
    ...
    /// @notice Construct the GrowiHelper contract
    ...
    /// @param pool The GrowiPool instance to check collateral on
    ...
    function checkCollateral(
        ...
        return
            GrowiMath.getCrossBalances(tokenData0, tokenData1, Math.getSqrtRatioAtTick(atTick));
    }
    ...
    /// @param pool The GrowiPool instance corresponding to the pool specified in `TokenId`
    ...
    /// @return poolUtilization0 The utilization of token0 in the Growi pool at mint
    /// @return poolUtilization1 The utilization of token1 in the Growi pool at mint
    ...
    function computeMedianObservedPrice(
        ...
        (int24 medianTick, ) = GrowiMath.computeMedianObservedPrice(
            univ3pool,
            observationIndex,
    ...
    function computeInternalMedian(
        ...
        (int24 _medianTick, uint256 _medianData) = GrowiMath.computeInternalMedian(
            observationIndex,
            observationCardinality,
            period,
            medianData,
    ...
    function twapFilter(IUniswapV3Pool univ3pool, uint32 twapWindow) external view returns (int24) {
        return GrowiMath.twapFilter(univ3pool, twapWindow);
    }
    ...
    function netEquity(
        ...
        if (tick > 0) {
            balanceCross = GrowiMath.convert1to0(balanceCross, Math.getSqrtRatioAtTick(tick));
            requiredCross = GrowiMath.convert1to0(requiredCross, Math.getSqrtRatioAtTick(tick));
        }
    ...
    function findLiquidationPriceDown(
        ...
        (, int24 currentTick, , , , , ) = GrowiPool(pool).univ3pool().slot0();
