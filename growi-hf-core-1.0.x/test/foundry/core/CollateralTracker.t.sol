...
// Growi Core
import {GrowiPool} from "@contracts/GrowiPool.sol";
import {SemiFungiblePositionManager} from "@contracts/SemiFungiblePositionManager.sol";
import {CollateralTracker} from "@contracts/CollateralTracker.sol";
import {GrowiHelper} from "@test_periphery/GrowiHelper.sol";

// Growi Libraries
import {GrowiMath} from "@libraries/GrowiMath.sol";
import {Math} from "@libraries/GrowiMath.sol";
...
contract CollateralTrackerHarness is CollateralTracker, PositionUtils, MiniPositionManager {
    ...
    // view deployer (growi pool)
    function growiPool() external view returns (GrowiPool) {
        return s_growiPool;
    }
...
// Inherits all of GrowiPool's functionality, however uses a modified version of startPool
// which enables us to use our modified CollateralTracker harness that exposes internal data
contract GrowiPoolHarness is GrowiPool {
    constructor(SemiFungiblePositionManager _SFPM) GrowiPool(_SFPM) {}
...
contract CollateralTrackerTest is Test, PositionUtils {
    ...
    // Current instance of Growi Pool, CollateralTokens, and SFPM
    GrowiPoolHarness growiPool;
    address growiPoolAddress;
    GrowiHelper growiHelper;
    ...
    function _initWorld(uint256 seed) internal {
        ...
        _deployCustomGrowiPool(token0, token1, pool);
    }

    function _cacheWorldState(IUniswapV3Pool _pool) internal {
        pool = _pool;
        poolId = GrowiMath.getPoolId(address(_pool), _pool.tickSpacing());
        token0 = _pool.token0();
        token1 = _pool.token1();
    ...
    function _deployCustomGrowiPool(
        address _token0,
        address _token1,
        IUniswapV3Pool uniswapPool
    ) internal {
        ...
        growiHelper = new GrowiHelper(SemiFungiblePositionManager(sfpm));

        // deploy modified Growi pool
        growiPool = new GrowiPoolHarness(sfpm);

        // initalize Growi Pool
        growiPool.modifiedStartPool(_token0, _token1, uniswapPool);

        collateralToken0 = CollateralTrackerHarness(address(growiPool.collateralToken0()));
        collateralToken1 = CollateralTrackerHarness(address(growiPool.collateralToken1()));

        // store growi pool address
        growiPoolAddress = address(growiPool);
    }
    ...
    function _mockMaxDeposit(address recipient) internal {
        ...
        deal(
            token0,
            address(growiPool),
            IERC20Partial(token0).balanceOf(growiPoolAddress) + initialMockTokens
        );
        deal(
            token1,
            address(growiPool),
            IERC20Partial(token1).balanceOf(growiPoolAddress) + initialMockTokens
        );
    }

    //@note move this and growiPool helper into position utils
    /*//////////////////////////////////////////////////////////////
                                HELPERS
    //////////////////////////////////////////////////////////////*/
    ...
    function test_Success_StartToken_virtualShares() public {
        ...
        ct.startToken(false, token0, token1, fee, growiPool);
    ...
    function test_Fail_startToken_alreadyInitializedToken(uint256 x) public {
        ...
        // initialize the token
        collateralToken0.startToken(false, token0, token1, fee, GrowiPool(address(0)));
    ...
    function test_Success_deposit(uint256 x, uint104 assets) public {
        ...
        // check if the growi pool got transferred the correct underlying assets
        assertEq(assets, IERC20Partial(underlyingToken0).balanceOf(address(growiPool)));
        assertEq(assets, IERC20Partial(underlyingToken1).balanceOf(address(growiPool)));
    }
    ...
    function test_Success_mint(uint256 x, uint104 shares) public {
        ...
        // check if the growi pool got transferred the correct underlying assets
        assertEq(returnedAssets0, IERC20Partial(underlyingToken0).balanceOf(address(growiPool)));
        assertEq(returnedAssets1, IERC20Partial(underlyingToken1).balanceOf(address(growiPool)));
    }
    ...
    function test_Fail_mint_DepositTooLarge(uint256 x, uint256 shares) public {
        ...
        IERC20Partial(token0).transfer(address(growiPool), 2 ** 64);
        IERC20Partial(token1).transfer(address(growiPool), 2 ** 64);
    ...
    function test_Fail_transfer_positionCountNotZero(
        ...
        // deposit a significant amount of assets into the Growi pool
        _mockMaxDeposit(Bob);
    ...
    function test_Success_redeem(uint256 x, uint104 shares) public {
        ...
        uint256 debitedBalance1;
        {
            ...
            // Bob's asset balance after depositing to the Growi pool
        ...
        // check correct amount of assets were moved from the the Growi Pool to LP
    ...
    function test_success_delegate_virtual(uint256 x) public {
        ...
        growiPool.delegate(Alice, collateralToken0);
        growiPool.delegate(Alice, collateralToken1);
    ...
    function test_success_revoke_virtual(uint256 x) public {
        ...
        growiPool.revoke(Alice, collateralToken0);
        growiPool.revoke(Alice, collateralToken1);
    ...
    function test_success_refund_positive(uint256 x, uint104 shares) public {
        ...
        // invoke delegate transactions from the Growi pool
        growiPool.refund(
            Alice,
            Bob,
    ...
    function test_Fail_All_OnlyGrowiPool(uint256 x, address caller) public {
        _initWorld(x);
        vm.assume(caller != address(growiPool));

        vm.prank(caller);

        vm.expectRevert(Errors.NotGrowiPool.selector);
    ...
    function test_Success_collateralCheck_shortStrangle(
...

        {
            ...
            ($shortPremia, $longPremia, posBalanceArray) = growiPool
            ...
            (, uint64 poolUtilization0, uint64 poolUtilization1) = growiHelper
                .optionPositionInfo(growiPool, Alice, tokenId1);
...

        {
            ...
            (uint256 calcBalanceCross, uint256 calcThresholdCross) = GrowiMath.getCrossBalances(
                tokenData0,
                tokenData1,
                Math.getSqrtRatioAtTick(currentTick)
            );

            (balanceData0, thresholdData0) = growiHelper.checkCollateral(
                growiPool,
                Alice,
                currentTick,
                positionIdList1
    ...
    function test_Success_collateralCheck_ITMcallSpread_assetTT1(
...

        {
            ...
            (rangeDown0, rangeUp0) = GrowiMath.getRangesFromStrike(width, tickSpacing);

            (rangeDown1, rangeUp1) = GrowiMath.getRangesFromStrike(width1, tickSpacing);
    ...
    function test_Success_collateralCheck_sellCallMinUtilization(
        ...
        // check requirement at fuzzed tick
        {
            ...
            (, uint64 poolUtilization0, uint64 poolUtilization1) = growiHelper
                .optionPositionInfo(growiPool, Bob, tokenId);
        ...
        //check collateral output against growi pool at current tick
        {
            (, currentTick, , , , , ) = pool.slot0();
    ...
    function test_Success_exerciseCostRanges_OTMCall(
        ...
        // check requirement at fuzzed tick
        {
            ...
            (, int24 rangeUp) = GrowiMath.getRangesFromStrike(width, tickSpacing);

            // strike - rangeDown
            vm.assume(atTick < legLowerTick);

            (LeftRightSigned longAmounts, ) = GrowiMath.computeExercisedAmounts(
                tokenId1,
                positionSize0 / 4
            );
    ...
    function setUtilization(
        ...
        // totalAssets() = GrowiPoolBal - lockedFunds + inAMM
        //        totalAssets() = z + inAMM
        //
        //
        // DECIMALS = 10_000
        //
        // utilization = (inAMM * DECIMALS) / ((GrowiPoolBal - lockedFunds) + inAMM)
        //      z = (GrowiPoolBal - lockedFunds)
        //      utilization =  (inAMM * DECIMALS) / z + inAMM
        //      inAMM = (utilization * z) / (10_000 - utilization)
        //      inAMM / z = utilization / (10_000 - utilization)
        //
        // inAMM / (GrowiPoolBal - lockedFunds) = (utilization / (10_000(DECIMALS) - utilization))
        //
        // i.e utilization of 9_000
        //    inAMM / (GrowiPoolBal - lockedFunds) = 9_000 / 10_000 - 9_000
        //    inAMM / (GrowiPoolBal - lockedFunds) = 9
        ...
        //
        //-----------------------------------------------------------
        int128 _poolBalance = int128(int256(IERC20Partial(token).balanceOf(address(growiPool))));
        ...
        // inAMM = utilization * (GrowiPoolBal - lockedFunds) / (10_000 - utilization)
        ...
        deal(token, address(growiPool), uint128(_poolBalance));
    }
    ...
    function test_Success_maxRedeem(uint256 x, uint104 shares) public {
        ...
        if (growiPool.numberOfLegs(Bob) != 0) {
    ...
    function test_Success_availableAssets(uint256 x, uint256 balance) public {
        ...
        // set total balance of underlying asset in the Growi pool
    ...
    function _swapITM(int128 itm0, int128 itm1) internal {
        ...
        if ((itm0 != 0) && (itm1 != 0)) {
            ...
            int256 net0 = itm0 + GrowiMath.convert1to0(itm1, sqrtPriceX96);

            int256 net1 = itm1 + GrowiMath.convert0to1(itm0, sqrtPriceX96);
    ...
    function _tokensRequired(
        ...
        for (; i < maxLoop; ++i) {
            ...
            (int24 rangeDown, int24 rangeUp) = GrowiMath.getRangesFromStrike(
                _width,
                tickSpacing
            );
            ...
            {
                LeftRightUnsigned _amountsMoved = GrowiMath.getAmountsMoved(
                    _tokenId,
                    positionSize,
                    i
                );
        ...
        for (uint i; i < maxLoop; ++i) {
...

            {
                if (isLong == 1) {
                    // spread requirement
                    {
                        amountsMoved = GrowiMath.getAmountsMoved(_tokenId, positionSize, i);

                        amountsMovedPartner = GrowiMath.getAmountsMoved(
                            _tokenId,
                            positionSize,
                            partnerIndex
                        );
    ...
    function _strangleTokensRequired(
        ...
        for (uint i; i < maxLoop; ++i) {
            ...
            (, rangeUp0) = GrowiMath.getRangesFromStrike(width, tickSpacing);
