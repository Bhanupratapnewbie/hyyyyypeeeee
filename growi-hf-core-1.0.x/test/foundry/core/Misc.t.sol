...
contract Misctest is Test, PositionUtils {
    ...
    function setUp() public {
        ...
        poolReference = address(new GrowiPool(sfpm));
    ...
    function _createGrowiPool() internal {
        vm.startPrank(Deployer);
    ...
    function test_gas_MaxPositions_short_packed() public {
        ...
        for (uint256 i = 0; i < positionCount; i++) {
            TokenId posId = TokenId
                .wrap(0)
                .addPoolId(GrowiMath.getPoolId(address(uniPool), uniPool.tickSpacing()))
                .addLeg({
                    legIndex: 0,
                    _optionRatio: 1,
                    _asset: 1,
    ...
    function test_TickLimits_Initial(
        ...
        (int24 tickLimitLower, int24 tickLimitUpper) = sfpm.getEnforcedTickLimits(
            GrowiMath.getPoolId(address(uniPool), uniPool.tickSpacing())
        );
    ...
    function test_success_exercise_crossDelegate() public {
        ...
        GrowiMath.twapFilter(uniPool, 600);
    ...
    function test_success_NotionalRounding() public {
        ...
        (uint256 balanceCross, uint256 requiredCross) = GrowiMath.getCrossBalances(
            tokenData0,
            tokenData1,
    ...
    function test_success_liquidation_fuzzedSwapITM(uint256[4] memory prices) public {
        ...
        for (uint256 i; i < 4; ++i) {
            ...
            amountsMoved = GrowiMath.getAmountsMoved(tokenId, 3000, 0);
    ...
    function test_Fail_DivergentSolvencyCheck_liquidation() public {
        ...
        twapTick = GrowiMath.twapFilter(uniPool, 600);
        {
            (totalCollateralBalance0, totalCollateralRequired0) = ph.checkCollateral(
                pp,
                Bob,
