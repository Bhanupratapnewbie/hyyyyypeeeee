...
contract GrowiFactoryHarness is GrowiFactory {
    constructor(
...
contract GrowiFactoryTest is Test {
    // the instance of the Growi Factory we are testing
    GrowiFactoryHarness growiFactory;
    ...
    // These assets are used to mint 'dead shares' to the Growi Factory
    ...
    function _initalizeWorldState(IUniswapV3Pool _pool) internal {
        ...
        IERC20Partial(token0).approve(address(growiFactory), INITIAL_MOCK_TOKENS);
        IERC20Partial(token1).approve(address(growiFactory), INITIAL_MOCK_TOKENS);
    ...
    function setUp() public {
        ...
        // Deploy factory
        growiFactory = new GrowiFactoryHarness(
            sfpm,
            V3FACTORY,
            address(new GrowiPool(sfpm)),
    ...
    function test_Success_deployNewPool(uint256 x, uint96 salt) public {
        ...
        // Compute clone determinsitic Growi Factory address
        address poolReference = growiFactory.getPoolReference();
        address preComputedPool = predictDeterministicAddress(
            ...
            address(growiFactory)
        );

        {
            // Deploy pool
            // links the Uniswap V3 pool to the Growi pool
            GrowiPool deployedPool = growiFactory.deployNewPool(token0, token1, fee, salt);
            ...
            // check if pool is linked to the correct growi pool in factory
            assertEq(address(growiFactory.getGrowiPool(pool)), address(deployedPool));
            // see if correct pool was linked in the growiPool
            IUniswapV3Pool linkedPool = GrowiPool(preComputedPool).univ3pool();
            address linkedPoolAddress = address(GrowiPool(preComputedPool).univ3pool());
    ...
    // Revert if trying to deploy a Growi Pool ontop of an invalid Uniswap Pool
    function test_Fail_deployinvalidPool() public {
        ...
        growiFactory.deployNewPool(token0, token1, fee, salt);
    }

    // Revert if deploying a Growi Pool that has already been initalized
    function test_Fail_deployExistingPool() public {
        ...
        unchecked {
            growiFactory.deployNewPool(token0, token1, fee, salt + 1);
        }
    }
    ...
    function test_Success_tokenURI_decodes() public {
        ...
        uint96 salt = uint96(block.timestamp);
        GrowiPool deployedPool = growiFactory.deployNewPool(token0, token1, fee, salt);
        uint256 growiPoolAddress = uint256(uint160(address(deployedPool)));
        bytes memory uri = bytes(growiFactory.tokenURI(growiPoolAddress));
    ...
    // Successfully reach or surpass target rarity and deploy a Growi pool with the mined 'bestSalt'
    function test_Success_mineTargetRarity(
        ...
        // mine pool address
        (uint96 bestSalt, uint256 highestRarity) = growiFactory.minePoolAddress(
            randomAddress,
            address(pool),
            nonce,
            50_000,
        ...
        assertEq(
            highestRarity,
            GrowiMath.numberOfLeadingHexZeros(
                predictDeterministicAddress(
                    growiFactory.getPoolReference(),
                    bytes32(
                        abi.encodePacked(
