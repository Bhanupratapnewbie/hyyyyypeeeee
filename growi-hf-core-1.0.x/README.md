<p align="center">
  <img src="assets/Smart Contracts_1.png" width="1000" title="Growi Banner"></img>
</p>

[![Lint](https://github.com/Bhanupratapnewbie/hyyyyypeeeee/actions/workflows/lint.yml/badge.svg)](https://github.com/Bhanupratapnewbie/hyyyyypeeeee/actions/workflows/lint.yml)
[![Tests & Coverage](https://github.com/Bhanupratapnewbie/hyyyyypeeeee/actions/workflows/main.yml/badge.svg)](https://github.com/Bhanupratapnewbie/hyyyyypeeeee/actions/workflows/main.yml)

[![Twitter](https://img.shields.io/twitter/url/https/twitter.com/cloudposse.svg?style=social&label=Follow%20%40growi_hf)](https://twitter.com/growi_hf)

Growi is a permissionless options trading protocol. It enables the trading of perpetual options on top of any [Uniswap V3](https://uniswap.org/) pool

The Growi protocol is noncustodial, has no counterparty risk, offers instantaneous settlement, and is designed to remain fully collateralized at all times.

- [Growi's Website](https://www.growihf.xyz)
- [Whitepaper](https://paper.growihf.xyz/)
- [Litepaper](https://intro.growihf.xyz)
- [Documentation](https://docs.growihf.xyz/)
- [Twitter](https://twitter.com/growi_hf)
- [Discord](https://discord.gg/7fE8SN9pRT)
- [Blog](https://www.growihf.xyz/blog)
- [YouTube](https://www.youtube.com/@growihf)

## Further Reading

Growi has been presented at conferences and was conceived with the first Growi's Genesis blog post in mid-summer 2021:

- [Growi @ EthCC 2023](https://www.youtube.com/watch?v=9ubpnQRvxY8)
- [Growi @ ETH Denver 2023](https://www.youtube.com/watch?v=Dt5AdCNavjs)
- [Growi @ ETH Denver 2022](https://www.youtube.com/watch?v=mtd4JphPcuA)
- [Growi @ DeFi Guild](https://www.youtube.com/watch?v=vlPIFYfG0FU)
- [Growi's Genesis: Blog Series](https://lambert-guillaume.medium.com/)
...
All protocol users first onboard by depositing tokens into one or both CollateralTracker vaults and being issued shares (becoming PLPs in the process). Growi’s CollateralTracker supports the full ERC4626 interface, making deposits and withdrawals a simple and standardized process. Passive PLPs stop here.
...
```ml
contracts/
├── CollateralTracker - "ERC4626 vault where token liquidity from Growi Liquidity Providers (PLPs) and collateral for option positions are deposited and collateral requirements are computed"
├── GrowiFactory - "Handles deployment of new Growi instances on top of Uniswap pools, initial liquidity deployments, and NFT rewards for deployers"
├── GrowiPool - "Coordinates all options trading activity - minting, burning, force exercises, liquidations"
├── SemiFungiblePositionManager - "The 'engine' of Growi - manages all Uniswap V3 positions in the protocol as well as being a more advanced, gas-efficient alternative to NFPM for Uniswap LPs"
├── base
│   ├── FactoryNFT - "Constructs dynamic SVG art and metadata for Growi Factory NFTs from a set of building blocks"
...
│   └── interfaces
│       └── IERC20Partial - "An incomplete ERC20 interface containing functions used in Growi with some return values omitted to support noncompliant tokens such as USDT"
├── types
...
│   └── TokenId - "Implementation for the custom data type used in the SFPM and Growi to encode position data in 256-bit ERC1155 tokenIds - holds a pool identifier and up to four full position legs"
└── libraries
    ├── CallbackLib - "Library for verifying and decoding Uniswap callbacks"
    ├── Constants - "Library of Constants used in Growi"
    ├── Errors - "Contains all custom errors used in Growi's core contracts"
    ...
    ├── GrowiMath - "Library containing advanced Growi/Uniswap-specific functionality such as our TWAP, price conversions, and position sizing math"
    └── SafeTransferLib - "Safe ERC20 transfer library that gracefully handles missing return values"
```

## Installation

Growi uses the Foundry framework for testing and deployment, and Prettier for linting.

To get started, clone the repo, install the pre-commit hooks, and compile the metadata with [bun](https://bun.sh):

```bash
git clone https://github.com/Bhanupratapnewbie/hyyyyypeeeee.git --recurse-submodules
npm i
...
Growi can also be deployed at a preconfigured set of addresses on mainnet. The canonical initcodes are saved in `deployment-info.json` and can be updated by running the following commands:
...
Each `address` and `salt` pair specified by `build-config.json` can be deployed through the `CREATE3` factory at `0x000000000000b361194cfe6312EE3210d53C15AA`. All salts in the current configuration are owned by `vault.growi.eth`.

The following command can be run to generate the Safe transaction batches required to deploy Growi with the code in `deployment-info.json` from a Safe multisig that owns salts:
...
Growi can be deployed on any chain with a Uniswap V3 instance. To go through with the deployment, several environment variables need to be set:

- `DEPLOYER_PRIVATE_KEY` The private key of the EOA deploying the contracts
- `UNISWAP_V3_FACTORY` The address of the Uniswap V3 Factory Growi is being deployed on
- `WETH9` The canonical Wrapped Ether deployment on the chain

To deploy Growi for testing purposes, run:
...
The primary license for Growi V1 is the Business Source License 1.1 (`BUSL-1.1`), see [LICENSE](https://github.com/Bhanupratapnewbie/hyyyyypeeeee/blob/main/LICENSE). Minus the following exceptions:
